...
## CCA-210 ae native projection layer

CCA-210 adds one operator-reviewed render plan and a pure repository-local
validator/renderer. It projects explicit values into four existing
ae-framework input shapes at one exact schema pin. Native output is data, not
upstream execution. Existing Context Pack files may be referenced and validated
from supplied exact bytes; they are never synthesized.

The plan, not the target repository or renderer defaults, owns claim/threat
selection and wording, criticality, A-level, native lanes/kinds, STRIDE/CWE,
scope, exclusions, and trust boundaries. The exact CCA property/threat IDs retain
...
assurance summary, review/finding, code map, execution plan, deployment/gate
policy, waiver, satisfaction, release, or publication artifact. It executes no
upstream system and adds no CLI, live probe, private-evidence handling, or
cryptographic implementation.

A successful render establishes only deterministic transformation and
conformance of the generated bytes to the exact pinned schemas. It does not
establish semantic compatibility, evidence existence/freshness/sufficiency,
claim satisfaction, product security, approval, certification, production
